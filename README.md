...
```lua
local replay = flagdash.BackendReplay.new(os.getenv("FLAGDASH_REPLAY_KEY"), { release = "2026.08" })
if replay:start() then
  replay:event("checkout_started", "action", { items = 2 })
  replay:breadcrumb("payment requested")
  replay:stop()
end
```

Only explicit events are recorded. Sensitive attribute keys are redacted, buffers are bounded, and `context_headers()` provides replay correlation. Use a `replays:write` key.
...
```lua
local release = client:ai_config_release("support-agent", "usr_123")
```
